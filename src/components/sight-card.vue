<template>
  <view class="page-sight">
    <view v-for="(item,index) in data" :key="index" class="sight-card">
      <navigator
        :url="item.activityid?'/pages/map/sight/main?activitySight=true&id='+ item.sceneryid+'&activityid='+item.activityid+'&startSceneryid='+(item.startSceneryid ? item.startSceneryid : -1)+'&endSceneryid='+(item.endSceneryid ? item.endSceneryid : -1)+'&groupid='+(item.group ?item.group[0]?item.group[0].groupid :-1: -1)+'&hasjoin='+hasjoin:'/pages/map/sight/main?&id='+ item.sceneryid"
      >
