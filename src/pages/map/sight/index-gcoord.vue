...
export default {
  ...
  async onShow () {
    ...
    if (this.$mp.query.activitySight === 'true') {
      ...
      let transAddress = gcoord.transform(
        [res.data.latitude, res.data.longitude], // 经纬度坐标
        gcoord.WGS84, // 当前坐标系
        gcoord.BD09 // 目标坐标系
      );
      console.log('transAddress-1111', transAddress)
      this.toAddress = transAddress[0] + ',' + transAddress[1];
    ...
    let transAddress = gcoord.transform(
      [this.centerY, this.centerX], // 经纬度坐标
      gcoord.WGS84, // 当前坐标系
      gcoord.BD09 // 目标坐标系
    );
    console.log('transAddress', transAddress)
    this.fromAddress = transAddress[0] + ',' + transAddress[1];
  ...
  methods: {
    ...
    formSubmit (e) {
      debugger
      if (!this.$mp.query.activitySight) {
        // this.onSign()
